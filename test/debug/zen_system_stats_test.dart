...
void main() {
  group('ZenSystemStats', () {
    ...
    test('should find all instances of specific type', () {
      ...
      var initialServices = ZenSystemStats.findAllInstancesOfType<TestService>();
      expect(initialServices, isEmpty, reason: 'Should start with no TestService instances');
      ...
      // Assert
      expect(services.length, equals(2), reason: 'Should find exactly 2 TestService instances');
    ...
    test('should find scope containing specific instance', () {
      // Verify clean state
      final initialServices = ZenSystemStats.findAllInstancesOfType<TestService>();
      expect(initialServices, isEmpty, reason: 'Should start with no TestService instances');
      ...
      // Assert
      expect(foundScope, isNotNull, reason: 'Should find a scope containing the service');
      expect(foundScope, equals(childScope), reason: 'Should find the child scope, not root scope');
    ...
    test('should handle empty system gracefully', () {
      ...
      final initialServices = ZenSystemStats.findAllInstancesOfType<TestService>();
      expect(initialServices, isEmpty, reason: 'Should start with clean system');

      // Act
      final stats = ZenSystemStats.getSystemStats();

      // Assert
      expect(stats['scopes']['total'], greaterThanOrEqualTo(1)); // At least root scope
      expect(stats['dependencies']['total'], equals(0)); // Should be exactly 0 for clean system
    });
    ...
    test('should find instances across complex scope hierarchy', () {
      ...
      final grandChildScope = childScope.createChild(name: 'isolated-grandchild-scope');
      grandChildScope.put<TestService>(grandChildService, tag: 'grandchild-tag');
      ...
      expect(allServices.length, equals(3),
          reason: 'Should find exactly 3 TestService instances across all scopes');
      expect(allServices, containsAll([rootService, childService, grandChildService]));
      ...
      final foundGrandChildScope = ZenSystemStats.findScopeContaining(grandChildService);
